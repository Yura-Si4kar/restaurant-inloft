...
import { ADD_EMPLOYEE, DELETE_EMPLOYEE, SET_PERSONNELS_LIST } from "../actions/personnelsActions";
import { ADD_MENU_ITEM_TO_ORDER_LIST, CALCULATE_THE_CLIENT, CHANGE_ELEMENT_RATING, CLEAR_ORDER_LIST, CLEAR_ORDER_LIST_FROM_THE_TABLE, OVERWRITE_ORDER_ITEM, REMOVE_ORDER_ITEM, SET_AUTH, SET_LOADING, SET_MENU_LIST, SET_ORDER_LIST, SET_SALES_LIST, SET_USER, TIE_THE_ORDER_TO_THE_TABLE } from "../actions/servicesActions";
...
function overwriteItem(state, item) {
    return {
        ...state,
            orders: state.orders.map((element) => element._id !== item._id ? element : item)
    }
}
...
function clearOrderFromTheTable(state, id) {
    return {
        ...state,
            tables: state.tables.map((table) => table._id === id ? { ...table, order: [] } : table),
    }
}
